...
| [`median_per_step.py`](scripts/median_per_step.py) | Extract median/mean/min/max `per_step_time` and the final loss from `worker_0.log` — the *wall-clock* readout, complementary to profile data (which is noisy and single-step). Defaults to dropping steps ≤ 50 as warmup. | `python3 scripts/median_per_step.py output/run/worker_0.log` |
| [`compare_loss.py`](scripts/compare_loss.py) | Bit-identity / tolerance-aware loss diff between two `worker_*.log`s. Use to verify a perf optimization didn't change loss math. Classifies relative diff as FP-order / moderate / real-change. | `python3 scripts/compare_loss.py before/worker_0.log after/worker_0.log` |
