...
For *writing* the PR/RFC body prose from templates + diff + evidence, use the companion
**`gitcode-rfc-pr-draft`** skill. This skill is the API mechanics: push, create, link,
retest, poll.
...
evidence, honest checkboxes. Draft via `gitcode-rfc-pr-draft`.
...
Draft the RFC body from `.gitcode/ISSUE_TEMPLATE/RFC-CN.yml` via `gitcode-rfc-pr-draft`
(title prefix `[RFC] `; sections 基本信息 / 概述 / 用例分析 / 方案设计 / 测试设计 /
缺点与风险 / 现有技术 / 未解决问题; put `!<pr>` in "相关 Issue/PR" for the reverse link).
